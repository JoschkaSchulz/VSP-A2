...
start(Starternummer) ->
	...
	logging(Filename, io_lib:format("ggt.cfg gelesen~n", [])),
	
	%Ping auf den Nameserver
	net_adm:ping(Nameservicenode),
	timer:sleep(1000),
	
	starter(Starternummer,Koordinatorname,Praktikumsgruppe,Teamnummer, Nameservicename).

starter(Starternummer,Koordinatorname,Praktikumsgruppe,Teamnummer, Nameservicename) ->
	Nameservice = global:whereis_name(Nameservicename),					%Namenservice holen
	
	%Logge das Starten
	{ok, Hostname} = inet:gethostname(),
	Filename = io_lib:format("ggtStarter_~p@~s.log", [Starternummer,Hostname]),
	logging(Filename, io_lib:format("Nameservice gebunden~n", [])),
	...
	receive 
	        ...
	        {pin,{Name,Node}} -> 
				Koordinator = {Name,Node},								%Speichere den Koordinator

				%Logge die verbindung mit dem Koordinator
				logging(Filename, io_lib:format("Koordinator (~s) gebunden~n", [Koordinatorname])),
				...
				receive
					{steeringval,ArbeitsZeit,TermZeit,GGTProzessnummer} ->	%Empfange Daten vom Koordinator
						...
						logging(Filename, io_lib:format("getsteeringval: ~p Arbeitszeit, ~p Terminierungszeit, ~p Anzahl der Prozesse~n", [ArbeitsZeit,TermZeit,GGTProzessnummer])),
